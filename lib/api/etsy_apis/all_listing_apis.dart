...
Future<dynamic> searchEtsyProductApi({
  ...
  var request = http.Request('GET', Uri.parse(
      'https://openapi.etsy.com/v3/application/listings/active?&keywords=$search'));
  request.headers.addAll(headers);
  http.StreamedResponse response = await request.send();
  var resp = jsonDecode(await response.stream.bytesToString());
  if (response.statusCode == 200){
...
Future<dynamic> getSingleListing({
  ...
  var request = http.Request('GET', Uri.parse('https://openapi.etsy.com/v3/application/listings/$listingId'));
  request.headers.addAll(headers);
  http.StreamedResponse response = await request.send();
  var resp = jsonDecode(await response.stream.bytesToString());
  if (response.statusCode == 200){
